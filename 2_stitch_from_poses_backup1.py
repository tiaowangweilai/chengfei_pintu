...
def run_global_stitcher(all_images, all_poses, output_path):
    ...
    avg_color = canvas_sum[valid_mask_bool] / canvas_count[valid_mask_bool]
    final_canvas[valid_mask_bool] = np.clip(avg_color, 0, 255).astype(np.uint8)

    # 2. 识别内部黑色虚线裂缝 (使用形态学闭运算)
    valid_mask_uint8 = (valid_mask_bool * 255).astype(np.uint8)
    kernel = np.ones((7, 7), np.uint8) # 7x7 扩张半径，足以覆盖虚线
    closed_mask = cv2.morphologyEx(valid_mask_uint8, cv2.MORPH_CLOSE, kernel)
    
    # 用 闭运算后的整体区块 减去 实际像素区块，得到的就是“内部裂缝”
    gap_mask = cv2.bitwise_xor(closed_mask, valid_mask_uint8)

    # 3. 使用周围点推测修补内部裂缝 (Inpaint)
    if np.any(gap_mask):
        # cv2.INPAINT_TELEA 基于快速行进算法，完美实现“利用周围点特征向内填充”
    ...
    # 开始向内收缩：只要四周边缘上存在任何“没有颜色”的像素，就继续向里切一刀
    while x1 < x2 and y1 < y2:
        ...
        # 如果四条边都是 100% 满像素，代表已经切到了完美的实体内部，跳出循环
    ...
    # 执行最终裁剪 (注意：保存的图片去掉了 Alpha 通道，纯 RGB 实体图片)
    cropped_final = final_canvas[y1:y2, x1:x2]
    
    if cropped_final.size > 0:
        cv2.imwrite(output_path, cropped_final)
        print(f"✅ 拼接、修复及四周边框裁剪完成！最终图像 ({cropped_final.shape[1]}x{cropped_final.shape[0]}) 已保存至: {output_path}")
    else:
